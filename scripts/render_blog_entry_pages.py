#!/usr/bin/env python3
"""
Сгенерировать отдельные HTML-страницы для записей блога (для индексирования).
Путь: blog/entry_[номер]_[slug].html
Хлебные крошки: Главная → Блог → Запись N: [заголовок]
Под записью — ссылка «Читать весь блог» на /blog/

Запуск: python3 scripts/render_blog_entry_pages.py [count]
По умолчанию count=5 (5 самых свежих непустых записей).
"""
import json
import re
import sys
...
_TRANS = str.maketrans(
    ...
    "abvgdeezziyklmnoprstufhccssyyyeuaABVGDEEZZIYKLMNOPRSTUFHCCSSYYYEUA",
)
...
def _load_entry_titles() -> dict[str, str]:
    global _ENTRY_TITLES_CACHE
    if _ENTRY_TITLES_CACHE is None:
        path = BLOG_DIR / "entry_titles.json"
        try:
            _ENTRY_TITLES_CACHE = json.loads(path.read_text(encoding="utf-8"))
        except Exception:
            _ENTRY_TITLES_CACHE = {}
...
def slug_from_title(title: str) -> str:
    """Транслитерация заголовка в slug (латиница, дефисы)."""
    s = title.translate(_TRANS)
    s = s.lower()
...
def entry_page_html(
    ...
    *,
    ...
    if locale == "en":
        brand = brand_mod.BRAND_EN
        logo_src = brand_mod.LOGO_EN
        logo_size = brand_mod.LOGO_SIZE_EN
        page_title = f"Blog post {entry_num}: {title} | {brand}"
        desc = (title + f". Motorhome blog — {brand}.")[:160]
        html_lang = "en"
        og_locale = "en_US"
        nav_menu = "Menu"
        bc_aria = "Breadcrumbs"
        ...
        brand = brand_mod.BRAND_RU
        logo_src = brand_mod.LOGO_RU
        logo_size = brand_mod.LOGO_SIZE_RU
        page_title = f"Запись в блоге {entry_num}: {title} | {brand}"
        desc = (title + f". Блог {brand}.")[:160]
        html_lang = "ru"
        og_locale = "ru_RU"
        nav_menu = "Меню"
...
{ru_redirect}  <script type="application/ld+json">{ld_breadcrumb}</script>
  ...
  <!-- VK Пиксель -->
  <script type="text/javascript">var _tmr=window._tmr||(window._tmr=[]);_tmr.push({{id:"3727349",type:"pageView",start:(new Date()).getTime()}});(function(d,w,id){{if(d.getElementById(id))return;var ts=d.createElement("script");ts.type="text/javascript";ts.async=true;ts.id=id;ts.src="https://top-fwz1.mail.ru/js/code.js";var f=function(){{var s=d.getElementsByTagName("script")[0];s.parentNode.insertBefore(ts,s);}};if(w.opera=="[object Opera]"){{d.addEventListener("DOMContentLoaded",f,false);}}else{{f();}}}})(document,window,"tmr-code");</script>
</head>
<body>
  <header class="site-header">
    <div class="container">
      <a href="../" class="logo"><img src="{asset}{logo_src}" alt="{blog_render.html.escape(brand)}" {logo_size}></a>
...
{article_wrapped}
  ...
  <footer class="site-footer">
    <div class="container">
      <div><h5>{blog_render.html.escape(brand)}</h5><a href="../legal/">{blog_render.html.escape(t_legal)}</a></div>
...
def delete_entry_page(entry_num: int) -> None:
    """Удалить индивидуальную страницу записи (при удалении поста в VK): файл, манифест, sitemap, BLOG_ENTRY_PAGES."""
    removed_name = None
    ...
    try:
        ...
        removed_name = manifest.pop(str(entry_num), None) or removed_name
        manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    except Exception:
        pass
    if removed_name and SITEMAP.exists():
        sitemap_text = SITEMAP.read_text(encoding="utf-8")
        sitemap_lines = [line for line in sitemap_text.splitlines() if removed_name not in line]
    ...
    if index_path.exists() and manifest_path.exists():
        try:
            ...
            if pattern_pages.search(idx_text):
                index_path.write_text(pattern_pages.sub(new_entry_pages, idx_text, count=1), encoding="utf-8")
        except Exception:
            pass
...
def main(count: int | None = None) -> None:
    ...
    if not blog_render.ENTRIES_DIR.exists():
        print("Папка entries не найдена:", blog_render.ENTRIES_DIR, file=sys.stderr)
        sys.exit(1)

    filenames = blog_render.get_initial_entry_filenames(count)
    if not filenames:
        print("Нет непустых записей.", file=sys.stderr)
        return
    ...
    for filename in filenames:
        ...
        path = blog_render.ENTRIES_DIR / filename
        try:
            data = json.loads(path.read_text(encoding="utf-8"))
        except Exception as e:
            print("Ошибка чтения", filename, e, file=sys.stderr)
            continue
        if blog_render.is_entry_empty(data):
            continue
    ...
    if not generated:
        print("Ни одна страница не создана.", file=sys.stderr)
        return

    # Манифест: номер записи → имя файла страницы (для ссылок с главной блога)
    manifest_path = BLOG_DIR / "entry_pages_manifest.json"
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except Exception:
        manifest = {}
    titles_path = BLOG_DIR / "entry_titles.json"
    try:
        titles = json.loads(titles_path.read_text(encoding="utf-8"))
    except Exception:
        titles = {}
    for entry_num, out_name, _, title in generated:
        manifest[str(entry_num)] = out_name
        if str(entry_num) not in titles:
            titles[str(entry_num)] = title
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    titles_path.write_text(json.dumps(titles, ensure_ascii=False, indent=2), encoding="utf-8")

    # Обновить BLOG_ENTRY_PAGES в index.html
    entry_pages = {f"entry_{k}.json": v for k, v in manifest.items()}
    new_entry_pages = "    var BLOG_ENTRY_PAGES = " + json.dumps(entry_pages, ensure_ascii=False) + ";"
    index_path = BLOG_DIR / "index.html"
    if index_path.exists():
        ...
        pattern_pages = re.compile(r"var BLOG_ENTRY_PAGES = \s*[^;]+;", re.DOTALL)
        if pattern_pages.search(idx_text):
            idx_text = pattern_pages.sub(new_entry_pages, idx_text, count=1)
    ...
    if SITEMAP.exists():
        ...
        if "</urlset>" in sitemap_text:
            sitemap_text = sitemap_text.replace("</urlset>", insert_block + "</urlset>")
            SITEMAP.write_text(sitemap_text, encoding="utf-8")
            print("Обновлён sitemap.xml, URL записей:", len(generated))
    else:
        print("sitemap.xml не найден, пропуск.", file=sys.stderr)

    print("Готово. Страниц записей:", len(generated))
